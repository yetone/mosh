...
namespace Terminal {
  ...
  public:
    ...
    {
      return ( rows == x.rows ) && ( window_title == x.window_title ) && ( clipboard  == x.clipboard ) && ( clipboard_seqnum == x.clipboard_seqnum ) && ( bell_count == x.bell_count ) && ( ds == x.ds ) && ( notification == x.notification ) && (sendfile == x.sendfile);
    }
  };
}
